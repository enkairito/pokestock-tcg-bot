...
NINTENDO_WEBSITE_URL = "https://wheresthatstock.com/nintendo"

# La tienda de marca de Nintendo en Amazon ES (amazon.es/stores/Nintendo/...)
# es puramente editorial: sus pestañas por franquicia (Mario, Zelda, Switch 2
# como "hub"...) son banners promocionales sin tarjetas de producto reales
# (0 [data-asin] comprobado a mano 2026-09-11), salvo "ACCESORIOS", que sí es
# una rejilla real. Para consolas y videojuegos no hay pestaña equivalente a
# "Novedades" de Yu-Gi-Oh!: en su lugar se usa el departamento propio de
# Amazon "Videojuegos", filtrando por nodo de categoría (Nintendo Switch 2 /
# Nintendo Switch) + búsqueda de texto — mismo patrón que ya usan los
# marketplaces UK/US de check_stock.py (búsqueda + rh=n%3A<nodo>). Confirmado
# 2026-09-11: el nodo devuelve una rejilla real (57 tarjetas), mezcla de
# consolas, juegos físicos y accesorios que categorize_nintendo() separa.
NINTENDO_MARKETPLACE = {
    "code": "ES",
    "domain": "amazon.es",
    "flag": "🇪🇸",
...
STATE_FILE = Path(__file__).parent / "state_nintendo.json"
SNAPSHOT_FILE = Path(__file__).parent / "nintendo_snapshot.json"
ACCESORIOS_SNAPSHOT_FILE = Path(__file__).parent / "accesorios_nintendo_snapshot.json"


ACCESSORY_NAME_KEYWORDS = [
    "funda", "estuche", "sleeve", "case", "protector de pantalla", "mando",
    "controller", "controlador", "gamepad", "joy-con", "joy con", "volante",
    "auriculares", "tarjeta de memoria", "microsd", "cargador", "grip",
    "soporte", "cable", "adaptador",
]
...
JUNK_NAME_KEYWORDS = [
    ...
    "figura", "poster", "póster", "taza", "camiseta", "ocarina de cer",
]
...
def save_snapshot(products):
    write_snapshot(SNAPSHOT_FILE, products, game='Nintendo')
...
async def main():
    ...
    nintendo_products = {}
    accessory_products = {}
    marketplace = NINTENDO_MARKETPLACE
    ...
    async with async_playwright() as p:
        ...
        for label, url in marketplace["pages"]:
            ...
            try:
                ...
                print(f"⚠️ No se pudo cargar la página de Nintendo ({label}), se omite esta vez: {e!r}")
        ...
        for asin, info in marketplace_products.items():
            ...
            if is_accessory(info["name"]):
                info["categories"] = ["Nintendo"]
                accessory_products[key] = info
                continue
            if is_excluded_by_name(info["name"]):
                continue

            category = categorize_nintendo(info["name"])
            if category is None:
                print(f"⏭️ Sin categoría reconocida, se descarta: {info['name'][:70]}")
                continue
            info["categories"] = [category]
            nintendo_products[key] = info

        await browser.close()

    all_products = {**nintendo_products, **accessory_products}
    print(f"📦 Total combinado: {len(nintendo_products)} productos Nintendo + {len(accessory_products)} accesorios")
    ...
    for key, info in all_products.items():
        ...
        # Los accesorios no generan avisos — igual que check_yugioh.py, solo
        # alimentan la web.
        is_main_product = key in nintendo_products
        ...
        if status_changed or stock_decreased or price_decreased:
            ...
            website_line = f'🌐 <a href="{NINTENDO_WEBSITE_URL}">Ver todo el stock de Nintendo</a>'
    ...
    save_state(state)
    save_snapshot(nintendo_products)
    write_snapshot(ACCESORIOS_SNAPSHOT_FILE, accessory_products)
    print("✅ Comprobación de Nintendo completada.")
