...
PLAYSTATION_WEBSITE_URL = "https://wheresthatstock.com/playstation"

# Mismo patrón que check_nintendo.py: la tienda de marca de PlayStation en
# Amazon ES es editorial, sin rejilla de productos aprovechable. Se usa en
# su lugar el departamento "Videojuegos" de Amazon filtrado por nodo de
# categoría (PS5 / PS4) + búsqueda de texto. Confirmado 2026-09-11: el nodo
# de PS5 devuelve productos reales (consola, mandos, auriculares, VR2).
PLAYSTATION_MARKETPLACE = {
    "code": "ES",
    "domain": "amazon.es",
    "flag": "🇪🇸",
...
STATE_FILE = Path(__file__).parent / "state_playstation.json"
SNAPSHOT_FILE = Path(__file__).parent / "playstation_snapshot.json"
ACCESORIOS_SNAPSHOT_FILE = Path(__file__).parent / "accesorios_playstation_snapshot.json"
...
ACCESSORY_NAME_KEYWORDS = [
    ...
    "auriculares", "gafas de realidad virtual", "vr2", "tarjeta de memoria",
    "microsd", "cargador", "grip", "soporte", "base de carga", "cable",
    "adaptador",
]
...
def save_snapshot(products):
    write_snapshot(SNAPSHOT_FILE, products, game='PlayStation')
...
async def main():
    ...
    accessory_products = {}
    marketplace = PLAYSTATION_MARKETPLACE
    ...
    async with async_playwright() as p:
        ...
        for label, url in marketplace["pages"]:
            ...
            try:
                ...
                print(f"⚠️ No se pudo cargar la página de PlayStation ({label}), se omite esta vez: {e!r}")
        ...
        for asin, info in marketplace_products.items():
            ...
            if is_accessory(info["name"]):
                info["categories"] = ["PlayStation"]
                accessory_products[key] = info
                continue
            if is_excluded_by_name(info["name"]):
                continue

            category = categorize_playstation(info["name"])
            if category is None:
                print(f"⏭️ Sin categoría reconocida, se descarta: {info['name'][:70]}")
                continue
            info["categories"] = [category]
            playstation_products[key] = info

        await browser.close()

    all_products = {**playstation_products, **accessory_products}
    print(f"📦 Total combinado: {len(playstation_products)} productos PlayStation + {len(accessory_products)} accesorios")
    ...
    for key, info in all_products.items():
        ...
        is_main_product = key in playstation_products
        ...
        if status_changed or stock_decreased or price_decreased:
            ...
            website_line = f'🌐 <a href="{PLAYSTATION_WEBSITE_URL}">Ver todo el stock de PlayStation</a>'
    ...
    save_state(state)
    save_snapshot(playstation_products)
    write_snapshot(ACCESORIOS_SNAPSHOT_FILE, accessory_products)
    print("✅ Comprobación de PlayStation completada.")
