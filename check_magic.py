...
ACCESORIOS_SNAPSHOT_FILE = Path(__file__).parent / "accesorios_magic_snapshot.json"
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for asin, info in marketplace_products.items():
            ...
            magic_products[key] = info

        await browser.close()

    all_products = {**magic_products, **accessory_products}
    print(f"📦 Total combinado: {len(magic_products)} productos TCG + {len(accessory_products)} accesorios")
    ...
    for key, info in all_products.items():
        ...
        is_tcg_card = key in magic_products
